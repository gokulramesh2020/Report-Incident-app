sap.ui.define([
	...
	return sap.ui.controller("customer.reportincidntapp.variant.controller.CreateIncidentCustom", {
		   formatter: f,
		   XMLView: X,
		   onInit: function () {
			//strt
	// 		 var oOccModel = new sap.ui.model.json.JSONModel({
    //     occTypes: []
    // });
    // this.getView().setModel(oOccModel, "createView");
	  var oGseModel = new sap.ui.model.json.JSONModel({
        visible: false
    });
    this.getView().setModel(oGseModel, "gse");
...
setTimeout(function () {
   ...
   this.byId("incidentClassRB").setSelectedIndex(0); // GOR
   this.onClassificationChange({
      getSource: () => this.byId("incidentClassRB")
   });
	...
	//ends.
		       ...
		       this.setModel(this._oViewModel, C.MODEL.CREATE_MODEL.NAME);
		   ...
		   // our cutom code starts below
		onClassificationChange: function (oEvent) {

    var sKey = oEvent.getSource().getSelectedButton().getText();
    var oSelect = this.byId("occTypeSelect");
...
_toggleGseSection: function (sType) {
   ...
   this.getView().getModel("gse").setProperty("/visible", sType === "GSE");
   this.getView().getModel("gse1").setProperty("/visible", sType === "GSE1");
   this.getView().getModel("gse2").setProperty("/visible", sType === "GSE2");
   this.getView().getModel("gse3").setProperty("/visible", sType === "GSE3");
   this.getView().getModel("gse4").setProperty("/visible", sType === "GSE4");
   this.getView().getModel("jse").setProperty("/visible", sType === "JSE");
   this.getView().getModel("jse1").setProperty("/visible", sType === "JSE1");
   this.getView().getModel("jse2").setProperty("/visible", sType === "JSE2");
   this.getView().getModel("jse3").setProperty("/visible", sType === "JSE3");
   this.getView().getModel("kse").setProperty("/visible", sType === "kse");
   this.getView().getModel("hse").setProperty("/visible", sType === "hSE");


},
onAircraftSelect:function(e){
   this.getView().getModel("hse")
   .setProperty("/aircraft", e.getParameter("selectedIndex") === 0);
},

onPassengerSelect:function(e){
   this.getView().getModel("hse")
   .setProperty("/pax", e.getParameter("selectedIndex") === 0);
},

onOccTypeChange: function (oEvent) {
   this._toggleGseSection(oEvent.getSource().getSelectedKey());
},
